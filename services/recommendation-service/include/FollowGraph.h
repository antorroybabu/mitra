...
public:
 ...
 [[nodiscard]] const std::unordered_set<std::uint64_t>* creators(std::uint64_t user) const { auto it=following_.find(user); return it==following_.end()?nullptr:&it->second; }
private: std::unordered_map<std::uint64_t,std::unordered_set<std::uint64_t>> following_;
};
}
